HELP_TEXT = (
    "ℹ️ <b>Справка по боту</b>\n\n"
    "Бот показывает расписание РЭУ им. Плеханова с сайта rasp.rea.ru.\n\n"
    "<b>Главное меню:</b>\n"
    "📅 <b>Сегодня / Завтра</b> — расписание на выбранный день\n"
    "📋 <b>Вся неделя</b> — расписание на текущую неделю\n"
    "📆 <b>На дату</b> — расписание на конкретный день: введи дату в формате <code>ДД.ММ</code>\n"
    "📆 <b>Период</b> — расписание за несколько дней подряд: введи промежуток вида <code>24.05–27.05</code>\n"
    "🚪 <b>Свободные аудитории</b> — свободные кабинеты по корпусу, дню и паре\n"
    "⭐ <b>Отзывы о преподавателях</b> — рейтинг и отзывы студентов о преподавателе\n"
    "✏️ <b>Изменить группу/преподавателя</b> — выбрать другую группу или преподавателя\n"
    "⚙️ <b>Настройки рассылки</b> — управление автоматическими уведомлениями\n\n"
    "<b>Автоматическая рассылка:</b>\n"
    "🌅 <b>Утренняя</b> — расписание на сегодня в выбранное время (по умолч. 07:00). "
    "Включена по умолчанию. Сообщение закрепляется в чате.\n"
    "🌙 <b>Вечерняя</b> — расписание на завтра в выбранное время (по умолч. 20:00). "
    "Выключена по умолчанию.\n"
    "📋 <b>Еженедельная</b> — расписание на следующую неделю каждое воскресенье "
    "в выбранное время (по умолч. 18:00). Выключена по умолчанию.\n\n"
    "<b>Команды:</b>\n"
    "/start — главное меню\n"
    "/help — справка"
)

START_WELCOME = (
    "Привет! Я помогу следить за расписанием РЭУ им. Плеханова.\n\n"
    "Введи номер группы или ФИО преподавателя:"
)
ASK_GROUP_AGAIN = "Введи номер группы или ФИО преподавателя:"
SEARCH_PICK = "Выбери из найденного:"
SEARCH_EMPTY = "Ничего не нашлось. Попробуй другой запрос:"
SEARCH_ERROR = "⚠️ Не удалось связаться с сайтом. Попробуй позже."
GROUP_SAVED = "✅ Сохранено: <b>{name}</b>"
GROUP_NOT_SET = "Сначала выбери группу — нажми /start."

MAIN_MENU = "Главное меню:"

BTN_TODAY = "📅 Сегодня"
BTN_TOMORROW = "📅 Завтра"
BTN_WEEK = "📋 Вся неделя"
BTN_CHANGE_GROUP = "✏️ Изменить группу/преподавателя"
BTN_SETTINGS = "⚙️ Настройки рассылки"
BTN_FREE_ROOMS = "🚪 Свободные аудитории"
BTN_BACK = "← Назад"

ROOMS_PICK_BUILDING = "🚪 <b>Свободные аудитории</b>\n\nВыбери корпус:"
ROOMS_PICK_DAY = "🚪 <b>Свободные аудитории</b>\n\n{building}\n\nВыбери день:"
ROOMS_PICK_PERIOD = "🚪 <b>Свободные аудитории</b>\n\n{building} · {date}\n\nВыбери пару:"
ROOMS_ASK_DATE = "Введи дату в формате <code>ДД.ММ</code> (на 30 дней вперёд):"
ROOMS_LOADING = "Ищу свободные аудитории…"

BTN_REVIEWS = "⭐ Отзывы о преподавателях"
REVIEWS_ASK = "⭐ <b>Отзывы о преподавателях</b>\n\nВведи фамилию преподавателя:"
REVIEWS_PICK = "Выбери преподавателя:"
REVIEWS_EMPTY_SEARCH = "Преподаватель не найден. Попробуй другую фамилию:"
REVIEWS_LOADING = "Загружаю отзывы…"
REVIEWS_NO_REVIEWS = (
    "⭐ <b>{name}</b>\n\nПока нет отзывов об этом преподавателе."
)
REVIEWS_CARD = (
    "⭐ <b>{name}</b>\n\n"
    "Рейтинг: <b>{rating}</b> / 5\n"
    "Отзывов: <b>{count}</b>"
)
REVIEWS_STALE = "Список устарел — начни заново через меню."
BTN_SHOW_REVIEWS = "📖 Смотреть отзывы"
BTN_TO_RATING = "⭐ К рейтингу"
BTN_MENU = "🏠 Меню"

SETTINGS_TITLE = "⚙️ Настройки рассылки\n\n{morning_line}\n{evening_line}\n{weekly_line}"
MORNING_ON = "🌅 Утренняя (на сегодня): <b>вкл.</b> в <b>{time}</b>"
MORNING_OFF = "🌅 Утренняя (на сегодня): <b>выкл.</b>"
EVENING_ON = "🌙 Вечерняя (на завтра): <b>вкл.</b> в <b>{time}</b>"
EVENING_OFF = "🌙 Вечерняя (на завтра): <b>выкл.</b>"
WEEKLY_ON = "📋 Еженедельная (по воскр.): <b>вкл.</b> в <b>{time}</b>"
WEEKLY_OFF = "📋 Еженедельная (по воскр.): <b>выкл.</b>"

BTN_MORNING_ON = "🌅 Утренняя: вкл."
BTN_MORNING_OFF = "🌅 Утренняя: выкл."
BTN_EVENING_ON = "🌙 Вечерняя: вкл."
BTN_EVENING_OFF = "🌙 Вечерняя: выкл."
BTN_WEEKLY_ON = "📋 Еженедельная: вкл."
BTN_WEEKLY_OFF = "📋 Еженедельная: выкл."
BTN_SET_MORNING_TIME = "🕐 Время утренней"
BTN_SET_EVENING_TIME = "🕐 Время вечерней"
BTN_SET_WEEKLY_TIME = "🕐 Время еженедельной"

ASK_TIME = (
    "Введи время в формате ЧЧ:ММ\n"
    "(например, <code>07:30</code>):"
)
TIME_INVALID = "⚠️ Неверный формат. Введи время как ЧЧ:ММ, например <code>07:30</code>:"
TIME_SAVED = "✅ Время сохранено: <b>{time}</b>"

SCHEDULE_LOADING = "Загружаю расписание…"
SCHEDULE_ERROR = "⚠️ Сайт расписания временно недоступен. Попробуй позже."

BTN_BY_DATE = "📆 На дату"
BTN_BY_RANGE = "📆 Период"

ASK_DATE = (
    "Введи дату в формате <code>ДД.ММ</code> или <code>ДД.ММ.ГГГГ</code>\n"
    "(например, <code>24.05</code>):"
)
ASK_RANGE = (
    "Введи период в формате <code>ДД.ММ–ДД.ММ</code> или <code>ДД.ММ.ГГГГ–ДД.ММ.ГГГГ</code>\n"
    "(например, <code>24.05–27.05</code>):"
)
DATE_INVALID = (
    "⚠️ Неверный формат даты. Введи как <code>ДД.ММ</code> или <code>ДД.ММ.ГГГГ</code>:"
)
RANGE_INVALID = (
    "⚠️ Неверный формат. Введи период как <code>ДД.ММ–ДД.ММ</code>\n"
    "(например, <code>24.05–27.05</code>):"
)
RANGE_TOO_LONG = "⚠️ Максимальный период — 14 дней."
